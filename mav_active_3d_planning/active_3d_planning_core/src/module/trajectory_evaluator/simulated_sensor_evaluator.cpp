...
void SimulatedSensorEvaluator::setupFromParamMap(Module::ParamMap* param_map) {
  ...
  setParam<bool>(param_map, "visualize_sensor_view", &p_visualize_sensor_view_,
                 false);
  setParam<bool>(param_map, "visualize_visible_voxel",
                 &p_visualize_visible_voxel_, false);
...
void SimulatedSensorEvaluator::visualizeTrajectoryValue(
  ...
  markers->addMarker(marker);
    ...
    VisualizationMarker marker_features;
    marker_features.type = VisualizationMarker::CUBE_LIST;
    marker_features.scale.x() = voxel_size;
    marker_features.scale.y() = voxel_size;
    marker_features.scale.z() = voxel_size;
    marker_features.color.r = 0.0;
    marker_features.color.g = 0.0;
    marker_features.color.b = 1.0;
    marker_features.color.a = 0.8;

    // points
    for (int i = 0; i < info->visible_voxels.size(); ++i) {
      marker_features.points.push_back(info->visible_voxels[i]);
    }
    markers->addMarker(marker_features);
  }
